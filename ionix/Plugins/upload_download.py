...
#
# Licensed under the MIT Public License, Version 1.d (the "License");
# you may not use this file except in compliance with the License.
#
# The entire source code is OSSRPL except
# 'download, uploadir, uploadas, upload' which is MPL
# License: MPL and OSSRPL
""" ionix module which contains everything related to \
...
from ionix.events import register


async def progress(current, total, event, start, type_of_ps, file_name=None):
    """Generic progress_callback for uploads reloads and downloads."""
    now = time.time()
    diff = now - start
...
@ionix.on(admin_cmd(pattern=r"uploadir (.*)", outgoing=True))
...
@ionix.on(admin_cmd(pattern=r"ul (.*)", outgoing=True))
...
@ionix.on(admin_cmd(pattern=r"uploadas(stream|vn|all) (.*)", outgoing=True))
